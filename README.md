...
from visitor_intel import (
    ...
    score_linkedin, compute_icp_score, score_to_fit_tier,
)

# From their IP address (automatic)
company = await fetch_company_intel("8.8.8.8")
ai_referrer = classify_ai_source("https://chatgpt.com/")

# From a form they filled in (optional — boosts accuracy significantly)
company_signals = await enrich_company_url("https://acmecorp.com")
url_score = company_signals_to_score(company_signals)
li_score = score_linkedin("https://linkedin.com/in/john-doe", job_title="VP of Sales")
...
score = compute_icp_score(
    ...
    pages_visited=3,
    is_returning=False,
    company_url_score=url_score,   # 0–25 from their company website
    linkedin_score=li_score,       # 0–20 from their LinkedIn
)
...
- **Rule-based ICP scoring** — 9-signal composite score, 0–100, zero LLM cost
...
Maximum: 100 (capped). Company URL and LinkedIn signals are optional — pass 0 if not available.
...
```bash
git clone https://github.com/perceptoai/visitor-intel
cd visitor-intel
pip install -e ".[dev]"
